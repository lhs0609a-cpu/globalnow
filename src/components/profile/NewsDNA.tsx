  ...
  return (
    <div className="surface p-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div>
          <h3 className="text-slate-400 t-body mb-3">카테고리 분포</h3>
        ...
        <div>
          <h3 className="text-slate-400 t-body mb-3">주요 소스</h3>
          <div className="space-y-2">
            {dna.sourceDistribution.map(src => (
              <div key={src.source} className="flex items-center justify-between p-2 bg-slate-700/30 rounded-lg">
                <span className="text-slate-100 t-body">{src.source}</span>
                <span className="text-slate-400 t-body">{src.percentage}%</span>
              </div>
            ))}
          </div>
        </div>
