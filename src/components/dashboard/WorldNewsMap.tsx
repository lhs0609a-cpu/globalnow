...
function MapButton({
  ...
  return (
    <button
      ...
      title={label}
      aria-label={label}
      className="flex h-7 w-7 items-center justify-center text-slate-400 transition-colors hover:bg-fill hover:text-slate-100"
    >
      {children}
    </button>
  );
    ...
    <Card>
        ...
        {QUICK_SELECT_COUNTRIES.map(({ code, center, zoom }) => {
          ...
          return (
            <button
              ...
              className={clsx(
                'flex flex-shrink-0 items-center gap-1.5 whitespace-nowrap rounded-md px-2.5 py-1.5 t-body-sm font-medium transition-colors',
                isActive
                  ? 'bg-fill text-slate-100'
                  : 'text-slate-400 hover:bg-fill-weak hover:text-slate-100'
              )}
            >
              <span className="t-body-sm leading-none">{info.flag}</span>
              {info.nameKo}
            </button>
          );
        })}
      ...
      <div className="flex flex-col lg:flex-row">
        ...
        <div className="relative min-w-0 flex-1 p-2 lg:p-3">
          ...
          <div className="absolute bottom-4 right-4 flex flex-col divide-y divide-line overflow-hidden rounded-lg border border-line-strong bg-slate-900/85 backdrop-blur-sm">
          ...
          {mapZoom > 1 && (
            <div className="tnum absolute left-4 top-4 rounded-md border border-line-strong bg-slate-900/85 px-2 py-1 t-meta-sm font-medium text-slate-400 backdrop-blur-sm">
              {mapZoom.toFixed(1)}×
            </div>
          )}
        </div>
        ...
        {selectedCountry && selectedInfo && (
          <div className="w-full flex-shrink-0 border-t border-line lg:w-80 lg:border-l lg:border-t-0">
            <div className="flex items-center gap-2.5 px-5 py-3.5">
              <span className="text-[1.0625rem] leading-none">{selectedInfo.flag}</span>
              <div className="min-w-0 flex-1">
                <h3 className="truncate t-title font-semibold text-slate-100">
                  {selectedInfo.nameKo}
                </h3>
                <p className="truncate t-meta-sm text-slate-500">
                  {selectedInfo.name}
                </p>
              </div>
              <button
                ...
                aria-label="닫기"
                className="flex h-7 w-7 items-center justify-center rounded-md text-slate-500 transition-colors hover:bg-fill-weak hover:text-slate-100"
              >
            ...
            <div className="max-h-[24rem] overflow-y-auto lg:max-h-[26rem]">
                ...
                <div className="space-y-4 px-5 py-4">
                  {[0, 1, 2].map(i => (
                    <div key={i} className="shimmer space-y-1.5 rounded">
                      ...
                      <div className="h-3 w-4/5 rounded bg-fill-weak" />
              ...
              {error && (
                <p className="px-5 py-8 text-center t-body-sm text-red-400">{error}</p>
              )}

              {!isLoading && !error && news.length === 0 && (
                <p className="px-5 py-8 text-center t-body-sm text-slate-500">
                  이 국가의 뉴스를 찾을 수 없습니다
                </p>
              )}
                ...
                <div className="divide-y divide-line">
                  {news.map((item, idx) => (
                    <a
                      ...
                      className="group block px-5 py-3 transition-colors hover:bg-fill-subtle"
                    >
                      {item.titleKo && (
                      ...
                      <p
                        className={clsx(
                          ...
                          item.titleKo
                            ? 'mt-1 text-slate-500'
                            : 'font-medium text-slate-100 transition-colors group-hover:text-accent-text'
                        )}
                      >
                        {item.title}
                      </p>
                      <div className="mt-2 flex items-center justify-between gap-2 t-meta-sm text-slate-500">
